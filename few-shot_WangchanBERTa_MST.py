...
if __name__ == '__main__':
    ...
    # model_name = "xlmr"
    # model_name = "mbert"
    args_params = f"{model_name} wisesight_sentiment {DIR}/Outputs/ {DIR}/Logs/ --batch_size 32 --seed {model_idx} --num_train_epochs 40"
    ...
    model = MODEL.model
    tokenizer.add_tokens(["<rep>", "<int>", "<msp>", "<lol>"])
    ...
    DATASETS = WangchanBERTaDataset("./Datasets/WisesightSentiment", MODEL.tokenizer, fewshot=True)
    dataset_split = DATASETS.preprocess(mst=True)
    ...
    trainer.save_model(f"{DIR}/few-shot-{model_name}_MST")
